...
{
    static POOL: OnceLock<Mutex<HashMap<SftpConnectionKey, Arc<Mutex<SftpConnection>>>>> =
        OnceLock::new();
    POOL.get_or_init(|| Mutex::new(HashMap::new()))
}

fn connect_sftp_connection(
...
pub fn connect_session(request: SftpConnectRequest) -> Result<(), String> {
    let connection_key = build_sftp_connection_key(
        &request.host,
        request.port,
        &request.username,
        &request.auth,
    ...
    F: FnMut(u64),
{
    ...
    if !is_dir {
        ...
        return copy_stream(&mut remote_file, &mut local_file, cancel_token, on_progress).map_err(
            |err| {
                if err == "transfer canceled" {
                    err
                } else {
    ...
    fs::create_dir_all(local_path).map_err(|err| {
        format!(
    ...
    F: FnMut(u64),
{
    ...
    let metadata = fs::metadata(local_path).map_err(|err| {
        format!(
            "failed to inspect local path {}: {err}",
            local_path.display()
        )
    })?;
    ...
    if metadata.is_file() {
        ...
        return copy_stream(&mut local_file, &mut remote_file, cancel_token, on_progress).map_err(
            |err| {
                if err == "transfer canceled" {
                    err
                } else {
                    format!(
                        "failed to upload local file {}: {err}",
                        local_path.display()
                    )
                }
            },
    ...
    if metadata.is_dir() {
        ...
        let children = fs::read_dir(local_path).map_err(|err| {
            format!(
                "failed to read local directory {}: {err}",
                local_path.display()
            )
        })?;
        ...
        for child in children {
            let child = child.map_err(|err| {
                format!(
                    "failed to iterate local directory {}: {err}",
                    local_path.display()
                )
            })?;
...
fn validate_overwrite_target(
    ...
    let local_metadata = fs::metadata(local_path).map_err(|err| {
        format!(
            "failed to inspect local path {}: {err}",
            local_path.display()
        )
