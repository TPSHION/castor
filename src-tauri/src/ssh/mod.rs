...
impl SshState {
  pub async fn connect(
    ...
    let runtime =
      tauri::async_runtime::spawn_blocking(move || connect_blocking(app, request, sessions_for_cleanup))
        .await
  ...
  pub fn list_sessions(&self) -> Vec<SessionSummary> {
    self
      ...
      .lock()
      .ok()
      .map(|sessions| sessions.values().map(|session| session.summary.clone()).collect())
      .unwrap_or_default()
  }
    ...
    thread::spawn(move || {
      ...
      loop {
        match reader.read(&mut output_buf) {
          Ok(0) => {
            let _ = runtime_tx.send(LocalRuntimeEvent::Ended("local session closed".to_string()));
            break;
          }
          Ok(bytes_read) => {
